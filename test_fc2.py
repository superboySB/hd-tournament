...
def load_params(path_dir):
  # policy_path = os.path.join(path_dir, 'params\\model\\policy.pkl')
  # with open(policy_path, 'rb') as f:
  #   model_params = cloudpickle.load(f)
  # aux_path = os.path.join(path_dir, 'params/ancillary.pkl')
...
def test_fc(path):
  ...
  for i in range(3000):
    red_msg = sim.get_obs(side='red')
    blue_msg = sim.get_obs(side='blue')
    if i % 500 == 0:
      delta = np.random.uniform([-100, -30, -40], [100, 30, 40], size=(len(scen['units']), 3))
      targets = {}
      for uid, (pid, info) in enumerate(red_msg.my_planes.items()):
        targets[pid] = model.get_target(info, delta[uid])
      for uid, (pid, info) in enumerate(blue_msg.my_planes.items()):
        targets[pid] = model.get_target(info, delta[uid+6])
    red_cmds = {}
    for pid, info in red_msg.my_planes.items():
      red_cmds[pid] = model.control_cmd(info, targets[pid])
      status = model.get_current_status(info)
      if np.all(targets[pid] - status < threshold):
        met_times[pid] += 1
    print(red_cmds)
    
    blue_cmds = {}
    for pid, info in blue_msg.my_planes.items():
      blue_cmds[pid] = model.control_cmd(info, targets[pid])
      status = model.get_current_status(info)
      if np.all(targets[pid] - status < threshold):
        met_times[pid] += 1
    sim.send_commands(red_cmds, cmd_side='red')
    sim.send_commands(blue_cmds, cmd_side='blue')
    sim.step()
  for pid, mt in met_times.items():
    print(pid, 'met times', mt)
