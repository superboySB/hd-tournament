...
# from core.mixin.actor import RMSStats
# from core.names import ANCILLARY, DEFAULT_ACTION, MODEL, PATH_SPLIT
# from core.typing import ModelPath, dict2AttrDict, AttrDict
# from core.builder import ElementsBuilder
# from tools.file import search_for_config
# from tools.pickle import restore_params
...
# from envs.hdfc import Side, HD_FlyControl
# from envs.hd import HD, EnvName
...
def load_params(path_dir):
  ...
  return model_params, aux_params
...
class FCModel:
  def __init__(self, path, obs_dim, is_action_discrete, action_dim):
    model_params, aux_params = load_params(path)
    self.policy = Policy(
    ...
    self.is_action_discrete = is_action_discrete
    obs_params = aux_params.obs[0]['obs']
    self.rms_mean = obs_params.mean
    self.rms_std = np.sqrt(obs_params.var + 1e-8)
...
def test_fc(path):
  ...
  obs = np.random.normal(size=(16))
  fc_cmd = model.control_cmd_from_obs(obs)
  print('fc cmd', fc_cmd)


if __name__ == '__main__':
  test_fc('fc_model\\discrete3')