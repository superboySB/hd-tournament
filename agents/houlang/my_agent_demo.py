...
class Agent(BaseAgent):
    def __init__(self, side):
        ...
        self.heat_zone_center_1 = Vector3(45e3, 0, -4000) # TODO: 左边是保护无人机用的假热区中心，真热区：Vector3(0, 0, -3000)
        self.heat_zone_center_2 = Vector3(-45e3, 0, -3000)
        ...
        if len(obs.awacs_infos):
            ...
            for enemy_id, tracks in self.enemy_plane_tracks.items():
                if len(tracks) < max_length:
                    raise ValueError("各存活敌机的位置记录不可能时间戳对不齐")
    ...
    def step(self, obs):
        ...
        for my_id, my_plane in obs.my_planes.items():
            ...
            if not my_plane.is_uav and self.use_fake_heat_zone:
                if self.calculate_distance_2d(my_plane,self.heat_zone_center_1) >= 20000:
            ...
            if self.phase == 1:
                target_pos = self.heat_zone_center_1
